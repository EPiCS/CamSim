...
STARTTIME=$SECONDS
BUILDDIR=build
JARNAME=buildSim.jar
LIBJAR=gnuprologjava-0.2.6.jar
BUILDCLASSPATH=$LIBJAR
BUILDMAIN=./src/simsim/epics/simSim/SimSim.java
...
if [ -f "$JARNAME" ]; then
    NEWER=`find ./src -newer ./buildSim.jar -name "*.java"`
    if [ -z "$NEWER" ]; then
        while true; do
...
function createmanifest {
    ...
    echo "Main-Class: simsim.epics.simSim.SimSim" > $MANIFESTNAME
