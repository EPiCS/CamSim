...
//    private static boolean MULTICAST_SMOOTH = true;
    ...
    private static final int AUCTION_DURATION = 1;
...
//    	}
    ...
    public IMessage receiveMessage(IMessage message) {
    	...
    	if(DELAY_COMMUNICATION > 0){
    		...
    		case AskConfidence: delayedCommunication.put(message, DELAY_COMMUNICATION); break;
	...
	//    	if(!VISION_ON_FOUND) //strenghen link at handover
...
//	private void checkConfidences_orig() {
...
//    	}
        ...
        if (DEBUG_CAM) {
            CmdLogger.println(this.camController.getName() + "->ALL: I'M LOOSING OBJECT ID:" + io.getFeatures() + "!! Can anyone take over? index " + index);
        }
//        if(index == 0){
//        	if(DEBUG_CAM){
			...
			if(highest > 0){
				...
				for(ICameraController icc : this.camController.getNeighbours()){
					...
					if(ratio > ran){
						sent ++;
		    			this.camController.sendMessage(name, mt, o);
		    			List<String> cams = advertised.get(io);
		    			if(cams != null){
...
**/
    ...
    /**
     * This method just checks if and object that has been lost by another camera has been found within this Field of View.
     */
//    private void checkIfSearchedIsVisible_orig() {
//    	
...
//                            double conf = this.getConfidence(visible);
//                            this.addOwnBidFor(visible, conf);
//                        } else {
//                        	this.camController.sendMessage(searcher.getName(), MessageType.Found, visible);
//                        }
//                    } else {
//                    	if(trackingPossible()){
    ...
    private void updateVisionGraph() {
    	if(!staticVG){
	        ...
	        for (Map.Entry<String, Double> e : visionGraph.entrySet()) {
	            ...
	            e.setValue( e.getValue() * EVAPORATIONRATE);
	            if (val < 0) {
...
//            utility += calculateValue(obj); 
    ...
    public double getObjectUtility(ITrObjectRepresentation tor){
    	...
    	visibility = this.getConfidence(tor);
	//      utility += calculateValue(obj); 
	  	resources = reservedResources.get(tor);
	      return visibility * classifier_confidence * enabled;// * resources;
    }
